...
const StudyApp = () => {
  ...
  const loadLocalFolder = async () => {
    try {
      ...
      const flatten = (items) => {
        let results = [];
        items.forEach(item => {
          if (['video', 'audio', 'pdf'].includes(item.type)) results.push(item);
          if (item.lessons) results = results.concat(flatten(item.lessons));
        });
        return results;
      };

      const flat = flatten(loadedCourses);
      setAllLessons(flat);
  ...
  const renderContent = () => {
    ...
    if (activeTab === 'list') {
      ...
      return (
        <div className="pt-32 px-4 md:px-12 pb-20 min-h-screen animate-fade-in">
    ...
    if (activeTab === 'courses') {
      return (
        <div className="pt-32 px-4 md:px-12 pb-20 min-h-screen animate-fade-in">
          ...
          {courses.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-[50vh] text-gray-400">
              <p className="text-xl mb-4">No courses loaded yet.</p>
              <button onClick={loadLocalFolder} className="bg-white text-black px-6 py-2 rounded font-bold hover:bg-gray-200 transition-colors">Open Folder to Start</button>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
        ...
        <div className="-mt-20 relative z-20 pl-4 md:pl-12 space-y-8">
          ...
          {courses.map(course => {
            const flatten = (items) => {
              ...
              items.forEach(item => {
                if (item.type) results.push(item);
                else if (item.lessons) results = results.concat(flatten(item.lessons));
              });
              return results;
            };
            ...
            return <NetflixRow key={course.path} title={formatTitle(course.name)} items={enhance(lessons)} onPlay={handlePlay} />;
          })}
        </div>
      </div>
    );
    ...
    <div className="min-h-screen bg-[#141414] text-white font-sans selection:bg-[#E50914] selection:text-white">
      ...
      {viewMode === 'watch' && currentLesson && (
        <div className="h-screen flex flex-col bg-black overflow-hidden relative">
          {/* Header */}
          <div className={clsx(
            "flex items-center justify-between px-4 py-3 z-50 shrink-0 w-full transition-all duration-300",
            !isStudyMode
          ...
          {isStudyMode ? (
            ...
            <div className="flex-1 flex w-full h-full overflow-hidden">
              <div className="w-[75%] h-full bg-black flex items-center justify-center border-r border-white/10 relative shadow-[0_0_50px_rgba(0,0,0,0.5)] z-10">
              ...
              <div className="w-[25%] h-full bg-[#111] border-l border-white/5">
                <RightPanel
            ...
            // --- CINEMA MODE ---
            <div className="flex-1 w-full h-full overflow-y-auto scrollbar-thin scrollbar-thumb-gray-800 bg-[#141414]">
              {/* Video Area */}
              <div className="w-full h-[85vh] bg-black flex items-center justify-center relative shadow-2xl z-10">
              ...
              {/* Content Below Video */}
              <div className="relative z-20 p-8 space-y-16 max-w-[1600px] mx-auto pb-32">
                {siblings.length > 0 && (
                  <div className="animate-fade-in-up">
                    <NetflixRow
                      title="More from this course"
                      items={siblings.map(s => ({ ...s, progress: progress[s.path] || 0, name: formatTitle(s.name) }))}
                      onPlay={handlePlay}
                    />
                  </div>
                )}
